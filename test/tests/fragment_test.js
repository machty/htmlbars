...
/*
 * wat; element helpers don't use placeholders, doy
test('hydrates a fragment with placeholder mustaches (ELEMENT)', function () {
  var ast = preprocess("<div {{foo}}></div>");
  var fragment = fragmentFor(ast).cloneNode(true);
  var hydrate = hydratorFor(ast);

  var helpers = {
    ATTRIBUTE: function (element, context, attrname, params, options) {
    }
  };
